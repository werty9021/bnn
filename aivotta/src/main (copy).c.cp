...
// volatile ushort32 B_out[1][6][8][1];// = {0}; // NEED TO SET TO ZERO EXPLICITLY, MISSING ld8/st8 FOR MEMSET!
// volatile uint32 v_input;
// void conv1(volatile unsigned* l1_output_ptr, volatile unsigned* l1_input_ptr, volatile unsigned * l1_weight_ptr){
void conv1(unsigned output_ptr, unsigned input_ptr, unsigned weight_ptr){
    // unsigned input_ptr = *l1_input_ptr;
    // unsigned weight_ptr = *l1_weight_ptr;
    // unsigned output_ptr = *l1_output_ptr;
    // Per layer
    ...
    // ushort32 B_out[TO_n][TO_y][TO_x][TO_c/32];
        ...
        for(int k_c = 0; k_c < K_c; k_c += TK_c){
            ...
            for(int o_n = 0; o_n < O_n; o_n += TO_n){
                for(int o_y = 0; o_y < O_y; o_y += TO_y){ //!
                    for(int o_x = 0; o_x < O_x; o_x += TO_x){ //!
                        // Output Buffer
                        ushort32 B_out[TO_n][TO_y][TO_x][TO_c/32] = {0}; // NEED TO SET TO ZERO EXPLICITLY, MISSING ld8/st8 FOR MEMSET!
                        // for(int k = 0; k < TO_n; k++){
                        //     for(int j = 0; j < TO_y; j++){
                        //         for(int i = 0; i < TO_x; i++){
                        //             B_out[k][j][i][0] = 0;
...
// void act1(volatile unsigned* a_output_ptr, volatile unsigned* a_input_ptr, volatile unsigned* a_threshold_ptr){
//     unsigned input_ptr = *a_input_ptr;
//     unsigned output_ptr = *a_output_ptr;
//     unsigned threshold_ptr = *a_threshold_ptr;
...
//                                     volatile ushort32 input_v = B_in[to_n][to_y][to_x][tk_c] << 1;
