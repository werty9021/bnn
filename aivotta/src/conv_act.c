...
void conv0_act0(unsigned output_ptr, unsigned input_ptr, unsigned weight_ptr){
    // Per layer
    const int K_c = 1; // Input maps
    const int O_c = 64; // Output channels
    // Tiling settings
    const int TO_c = 32; // Tile Output channels
    const int TO_y = 10; // Tile Output map height
    ...
    #if FPGA
    static _pmem short32 B_w[K_y][K_x][TK_c][TO_c/32];
    #else
            ...
            for(int k = 0; k < K_y; k++){
                for(int j = 0; j < K_x; j++){
                    for(int i = 0; i < TK_c; i++){
                        ...
                        #if FPGA
                        dma_queue_bursts(weight_ptr + ptr, (PMEM_OFFSET + (unsigned) &B_w[k][j][i][0]), TO_c, 2);
                        #else
                                            ...
                                            for(int tk_c = 0; tk_c < TK_c; tk_c++){
                                                ...
                                                for(int k_y = k_y_start; k_y < k_y_end; k_y++){
                                                    ...
                                                    for(int k_x = k_x_start; k_x < k_x_end; k_x++){
                                                        ...
                                                        short32 v_input;
                                                        _TCE_VBCAST16X32(B_in[to_n][to_y+k_y+1][to_x+k_x+1][tk_c], v_input);
                                                        _TCE_MAC16X32TO32X32(v_input, B_w[k_y][k_x][tk_c][to_c], B_out0, B_out0);
                                            }}}
                                            uint32_t B_out;
                                            _TCE_GE32X32TO32X1(B_out0, thresholds_v, B_out);
...
void conv1_act1(unsigned output_ptr, unsigned input_ptr, unsigned weight_ptr, unsigned threshold_ptr){
    ...
    #if FPGA
    dma_queue_bursts(threshold_ptr + 2*64, (DMEM_OFFSET + (unsigned) &B_thresholds3[0]), (O_c), 2);
    dma_queue_bursts(threshold_ptr + 4*64, (DMEM_OFFSET + (unsigned) &B_thresholds5[0]), (O_c), 2);
    #else
        for (int c = 0; c < O_c/32; c++){
            for (int pos = 0; pos < 32; pos++){
                uint16_t thres3 = B_thresholds[c][pos] > 3*64 ? B_thresholds[c][pos] - 3*64 : 0;
                _TCE_INSERTELEM16X32(B_thresholds3[c], thres3, pos, B_thresholds3[c]);
                uint16_t thres5 = B_thresholds[c][pos] > 5*64 ? B_thresholds[c][pos] - 5*64 : 0;
                _TCE_INSERTELEM16X32(B_thresholds5[c], thres5, pos, B_thresholds5[c]);
            }
        }
    #endif
    #endif
                                ...
                                for(int to_c = 0; to_c < TO_c/32; to_c++){
                                    for(int to_y = 0; to_y < TO_y; to_y++){
                                        for(int to_x = 0; to_x < TO_x; to_x++){
                                            ...
                                            uint32_t B_out;
                                            ushort32 thresholds_v = B_thresholds[o_c/32+to_c];
                                            if (((o_y + to_y) == 0 && (o_x + to_x) == 0) || ((o_y + to_y) == O_y-1 && (o_x + to_x) == 0) || 
                                                ((o_y + to_y) == 0 && (o_x + to_x) == O_x-1) || ((o_y + to_y) == O_y-1 && (o_x + to_x) == O_x-1)){
                                                thresholds_v = B_thresholds5[o_c/32+to_c];
                                            }else if ((o_x + to_x) == 0 || (o_x + to_x) == O_x-1 || (o_y + to_y) == 0 || (o_y + to_y) == O_y-1) {
                                                thresholds_v = B_thresholds3[o_c/32+to_c];
                                            }
                                            _TCE_GEU16X32TO32X1(B_out0 << 1, thresholds_v, B_out);
                                            unsigned ptr = 4*IND4((o_n + to_n), (o_y + to_y), O_y, (o_x + to_x), O_x, (o_c/32), O_c/32);
                                            #if FPGA
                                            _TCEAS_ST32("axi_as", output_ptr + ptr, B_out);
                                            #else
