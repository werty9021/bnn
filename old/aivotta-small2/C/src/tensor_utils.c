...
struct UInt16Tensor* uint16tensor_init(int N, int C, int H, int W, bool zeros){
    struct UInt16Tensor *tensor = (struct UInt16Tensor *) malloc(sizeof(struct UInt16Tensor));
    if (tensor == NULL){
        PRINTF("Malloc has failed allocating memory.\n");
        free(tensor);
        exit(EXIT_FAILURE);
    ...
    if (zeros == true) {
        tensor->data = (uint16_t*) calloc(TENSORSIZE(tensor), sizeof(uint16_t));
    } else {
        tensor->data = (uint16_t*) malloc(TENSORSIZE(tensor) * sizeof(uint16_t));
    }
...
struct IntTensor* inttensor_init(int N, int C, int H, int W, bool zeros){
    ...
    if (zeros == true) {
        tensor->data = (int32_t*) calloc(TENSORSIZE(tensor), sizeof(int32_t));
    } else {
        tensor->data = (int32_t*) malloc(TENSORSIZE(tensor) * sizeof(int32_t));
    }
...
struct Int16Tensor* int16tensor_init(int N, int C, int H, int W, bool zeros){
    ...
    if (zeros == true) {
        tensor->data = (int16_t*) calloc(TENSORSIZE(tensor), sizeof(int16_t));
    } else {
        tensor->data = (int16_t*) malloc(TENSORSIZE(tensor) * sizeof(int16_t));
    }
...
int16_t input_data[Hin][Win];// __attribute__((aligned(ALIGNNUM)));
int16_t l0_weights[Co][Kx][Kx] __attribute__((address_space(1)));// __dataBufAligned;
...
int16_t conv16_weights[Co][Kx][Kx] __dataBufAligned;

uint16_t act1_thresholds[Co] __dataBufAligned;
uint16_t act2_thresholds[Co] __dataBufAligned;
uint16_t act3_thresholds[Co] __dataBufAligned;
uint16_t act4_thresholds[Co] __dataBufAligned;
uint16_t act5_thresholds[Co] __dataBufAligned;
uint16_t act6_thresholds[Co] __dataBufAligned;
uint16_t act7_thresholds[Co] __dataBufAligned;
uint16_t act8_thresholds[Co] __dataBufAligned;
uint16_t act9_thresholds[Co] __dataBufAligned;
uint16_t act10_thresholds[Co] __dataBufAligned;
uint16_t act11_thresholds[Co] __dataBufAligned;
uint16_t act12_thresholds[Co] __dataBufAligned;
uint16_t act13_thresholds[Co] __dataBufAligned;
uint16_t act14_thresholds[Co] __dataBufAligned;
// float act15a_thresholds[Co] __dataBufAligned;
// float act15b_thresholds[Co] __dataBufAligned;
int16_t act15a_thresholds[Co] __dataBufAligned;
int16_t act15b_thresholds[Co] __dataBufAligned;

// output
float conv0_output[Co][Hin][Win] __dataBufAligned;
uint16_t conv1_output[Co][Hin][Win] __dataBufAligned;
uint16_t conv2_output[Co][Hin][Win] __dataBufAligned;
uint16_t conv3_output[Co][Hin][Win] __dataBufAligned;
uint16_t conv4_output[Co][Hin][Win] __dataBufAligned;
uint16_t conv5_output[Co][Hin][Win] __dataBufAligned;
uint16_t conv6_output[Co][Hin][Win] __dataBufAligned;
uint16_t conv7_output[Co][Hin][Win] __dataBufAligned;
uint16_t conv8_output[Co][Hin][Win] __dataBufAligned;
uint16_t conv9_output[Co][Hin][Win] __dataBufAligned;
uint16_t conv10_output[Co][Hin][Win] __dataBufAligned;
uint16_t conv11_output[Co][Hin][Win] __dataBufAligned;
uint16_t conv12_output[Co][Hin][Win] __dataBufAligned;
uint16_t conv13_output[Co][Hin][Win] __dataBufAligned;
uint16_t conv14_output[Co][Hin][Win] __dataBufAligned;
uint16_t conv15_output[Co][Hin][Win] __dataBufAligned;
...
int16_t act15i_output[Ci][Hin][Win] __dataBufAligned;
/*
c0
...
struct UInt16Tensor2* uint16tensor_from_file(int N, int C, int H, int W, char *path){
    struct UInt16Tensor2 *tensor = (struct UInt16Tensor2 *) malloc(sizeof(struct UInt16Tensor2));
    if (tensor == NULL){
        PRINTF("Malloc has failed allocating memory.\n");
        free(tensor);
        exit(EXIT_FAILURE);
    ...
    tensor->N = N; tensor->C = C; tensor->H = H; tensor->W = W; 
    #if __PLATFORM == _TCE
        __dataBufAligned uint16_t *var = NULL;
        if (strstr(path, "act1.bin") != NULL) {
            PRINTF("Copying act1...\n");
...
struct Int16Tensor2* int16tensor2_from_file(int N, int C, int H, int W, char *path){
    struct Int16Tensor2 *tensor = (struct Int16Tensor2 * ) malloc(sizeof(struct Int16Tensor2));
    if (tensor == NULL){
        PRINTF("Malloc has failed allocating memory.\n");
        free(tensor);
        exit(EXIT_FAILURE);
    ...
    tensor->N = N; tensor->C = C; tensor->H = H; tensor->W = W; 
    #if __PLATFORM == _TCE
        int16_t __attribute__((address_space(1)))* var = NULL;
        if (strstr(path, "conv0.bin") != NULL) {
            PRINTF("Copying conv0...\n");
            var = &l0_weights[0][0][0];
        } else if (strstr(path, "act15_a.bin") != NULL) {
...
void uint16tensor_to_file(struct UInt16Tensor *tensor, char *path){
    #if __PLATFORM == _TCE
        volatile __dataBufAligned uint16_t *var = NULL;
        if (strstr(path, "conv1.bin") != NULL){
...
void int16tensor_to_file(struct Int16Tensor *tensor, char *path){
    #if __PLATFORM == _TCE
        volatile __dataBufAligned int16_t *var = NULL;
        if (strstr(path, "act15i.bin") != NULL){
            PRINTF("Exporting act15...\n");
            var = &act15i_output[0][0][0];
        }
