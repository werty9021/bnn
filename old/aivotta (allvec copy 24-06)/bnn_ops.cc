/**
 * OSAL behavior definition file.
 */

#include "OSAL.hh"
#include <stdint.h>

/* XNOR POPCOUNT ACCUMULATOR */
OPERATION(XNORPOPCOUNTACC)
TRIGGER
    uint32_t x = UINT(1);
    uint32_t w = UINT(2);
    uint32_t prev_sum = UINT(3);

    IO(4) = prev_sum + __builtin_popcount(~(x ^ w));
    return true;
END_TRIGGER;
END_OPERATION(XNORPOPCOUNTACC)

/* SET_BIT */
OPERATION(SET_BIT)
TRIGGER
    uint32_t position = UINT(1);
    uint32_t bit_string = UINT(2);

    IO(3) = bit_string | (1 << (31 - position));
    return true;
END_TRIGGER;
END_OPERATION(SET_BIT)

/* GT32X32TO32 */
OPERATION(GT32X32TO32)
TRIGGER
    uint32_t bit_string = 0;
    
    for (int i = 0; i < 32; i++){
        bool val = (int32_t)SUBWORD32(1,i) >= (int32_t)SUBWORD32(2,i) ? 1 : 0;
        bit_string |= (val << (31 - i));
    }

    IO(3) = bit_string;
    return true;
END_TRIGGER;
END_OPERATION(GT32X32TO32)

/* GTU32X32TO32 */
OPERATION(GTU32X32TO32)
TRIGGER
    uint32_t bit_string = 0;
    
    for (int i = 0; i < 32; i++){
        bool val = (uint32_t)SUBWORD32(1,i) >= (uint32_t)SUBWORD32(2,i) ? 1 : 0;
        bit_string |= (val << (31 - i));
    }

    IO(3) = bit_string;
    return true;
END_TRIGGER;
END_OPERATION(GTU32X32TO32)

/* VREDUCE32X32 */
OPERATION(VREDUCE32X32)
TRIGGER
    int32_t sum = 0;
    for (int i = 0 ; i < 32; i++){
        sum += SUBWORD32(1,i);
    }
    IO(2) = sum;
END_TRIGGER;
END_OPERATION(VREDUCE32X32)